...
void getErrorOverAxis(Callable&& analytical_solution_f, const Math::SurfaceVectorField &j_e, const Math::SurfaceVectorField &j_m,
    ...
    std::transform(points.begin(), points.end(), std::back_inserter(amplitude),
                   [&](auto p) {
                       ...
                       Types::Vector3c an_value = std::forward<Callable>(analytical_solution_f)(p);
                       return (calculated_value - an_value).norm();
                   });
...
int main() {
    ...
    const std::string path = "/home/evgen/Education/MasterDegree/thesis/results/infinite_waveguide/mesh_convergance";
