...
Math::SurfaceVectorField matrixOperatorK(const Math::SurfaceVectorField &field, Types::complex_d k) {
    const MatrixXc A = Matrix::getMatrix(k, field.getManifold());
...
Math::SurfaceVectorField solve(const Mesh::SurfaceMesh &surfaceMesh, const Math::SurfaceVectorField &incidentField,
                               const complex_d k, const scalar tolerance) {
...
int main() {
    ...
    constexpr complex_d k = 4 * Math::Constants::PI<scalar>() * complex_d{1, 0.0};

    // EMW::Physics::planeWaveCase physics{Vector3d{0, 1, 0}, k, Vector3d{0, 0, -1}.normalized()};
    const auto rhs_field = getDeltaRHS(surfaceMesh);
    // расчет
    surfaceMesh.setName(surfaceMesh.getName() + "_" + std::to_string(k.real()) + "_+_i_" +
        std::to_string(k.imag()) + "_" + rhs_field.getName());
    ...
    auto analytical_solution = (4. / (k*k)) * (matrixOperatorK(rhs_field, k).normalCrossField());
...
//    analytical_solution.multiply(multiplier);
    ...
    auto numerical_solution = solve(surfaceMesh, rhs_field.crossWithNormalField(), k, 1e-5);
...
//    numerical_solution.multiply(multiplier);
    ...
    auto analytical_residual = matrixOperatorK(analytical_solution, k) - rhs_field.crossWithNormalField();
    analytical_residual.setName("residual");
//    analytical_residual.multiply(multiplier);
    auto numerical_residual = matrixOperatorK(numerical_solution, k) - rhs_field.crossWithNormalField();
