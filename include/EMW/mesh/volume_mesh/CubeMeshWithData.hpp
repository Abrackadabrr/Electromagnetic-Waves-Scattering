...
class CubeMeshWithData : public CubeMesh {
    ...
    template <typename Container> void loadScalarData(Container &&specific_scalar_data, const std::string &name) {
  ...
  public:
    ...
    CubeMeshWithData(const Types::Vector3d &minCorner, Types::scalar xs, Types::scalar ys, Types::scalar zs,
        ...
        : Base(minCorner, xs, ys, zs, nx, ny, nz), nodesCount(nodes_.size()), cellsCount(cells_.size()) {}
    ...
    void invokeScalarData(const std::string &name, Callable &&cell_function)
    requires std::is_invocable_v<Callable, CellsType>;
    template <typename Callable>
    void invokeScalarData(const std::string &name, Callable &&point_function)
    requires std::is_invocable_v<Callable, Types::point_t>;
    template <typename Quadrature, typename Callable>
    void smoothScalarData(const std::string &name, Callable &&point_function)
    requires std::is_invocable_v<Callable, Types::point_t>;

    template <typename Callable>
    void invokeVectorData(const std::string &name, Callable &&point_function)
    requires std::is_invocable_v<Callable, CellsType>;
    template <typename Callable>
    void invokeVectorData(const std::string &name, Callable &&point_function)
    requires std::is_invocable_v<Callable, Types::point_t>;
...
template <typename Callable>
void CubeMeshWithData::invokeScalarData(const std::string &name, Callable &&point_function)
requires std::is_invocable_v<Callable, Types::point_t>
{
...
template <typename Quadrature, typename Callable>
void CubeMeshWithData::smoothScalarData(const std::string &name, Callable &&point_function)
requires std::is_invocable_v<Callable, Types::point_t>
{
...
template <typename Callable>
void CubeMeshWithData::invokeVectorData(const std::string &name, Callable &&point_function)
requires std::is_invocable_v<Callable, Eigen::Matrix<double, 3, 1>>
{
