...
#define EMW_RESTORE_EIGEN_FFTW_DEFAULT
#undef EIGEN_FFTW_DEFAULT
#endif
#include "third_party/eigen/unsupported/Eigen/FFT"
#ifdef EMW_RESTORE_EIGEN_FFTW_DEFAULT
#define EIGEN_FFTW_DEFAULT
#undef EMW_RESTORE_EIGEN_FFTW_DEFAULT
#endif
 ...
 */
template <typename scalar_t>
class TripleToeplitz3x3FourierParallel {
public:
    using scalar_type = scalar_t;
    using complex_type = Types::complex_d;
    using vector_type = Types::VectorX<scalar_t>;
    ...
    TripleToeplitz3x3FourierParallel() = default;
...
public:
    explicit TripleToeplitz3x3FourierParallel(const tensor_type& levels)
        : levels_(levels) {
        init_layout_and_kernel_spectrum();
    }

    explicit TripleToeplitz3x3FourierParallel(tensor_type&& levels)
        : levels_(std::move(levels)) {
        init_layout_and_kernel_spectrum();
    }

    explicit TripleToeplitz3x3FourierParallel(const triple_toeplitz_block_type& matrix)
        : levels_(tensor_from_triple_toeplitz_block(matrix)) {
        init_layout_and_kernel_spectrum();
    }
    ...
    [[nodiscard]] vector_type matvec(const vector_type& vec) const {
        ...
        for (Types::index z = 0; z < nz_; ++z) {
            for (Types::index y = 0; y < ny_; ++y) {
                for (Types::index x = 0; x < nx_; ++x) {
                    ...
                    for (Types::index in_comp = 0; in_comp < block_size_; ++in_comp) {
                        vec_spectrum[in_comp][fft_index] =
        ...
        // Extract the physical convolution window.
        //
...
template <typename scalar_t>
[[nodiscard]] Types::VectorX<scalar_t> operator*(const TripleToeplitz3x3FourierParallel<scalar_t>& matrix,
                                                 const Types::VectorX<scalar_t>& vec) {
    return matrix.matvec(vec);
}
