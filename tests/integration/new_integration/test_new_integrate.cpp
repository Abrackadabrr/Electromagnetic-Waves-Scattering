...
struct Segment {
    ...
    scalar barycentric(scalar w) const {
        return w * a + (1 - w) * b;
    }
...
struct Triangle {
    ...
    [[nodiscard]] scalar mes() const {
        return (b - a).cross(b - c).norm() / 2;
    }

    point_t barycentric(const Containers::array<scalar, 2>& coord) const {
        return coord[0] * a + coord[1] * b + (1 - coord[0] - coord[1]) * c;
    }
};

TEST_F(NEW_GAUSSIAN_QUADRATURE, TEST_FOURTH_ORDER) {
    const double result = Math::Integration::Numerical::integrate<NewGL::GaussianPoints<1, 11>>(f, Segment{0, 1});
