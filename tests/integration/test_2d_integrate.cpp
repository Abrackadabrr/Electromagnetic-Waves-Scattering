...
#if 0
TEST_F(GAUSSIAN_QUADRATURE, TEST_CONSTANT) {
    const auto result = DefiniteIntegrals::integrate<DefiniteIntegrals::Quadrature<3, 3>>(f, {-1, -1}, {2, 2});
    ASSERT_EQ(4, result);
}
...
TEST_F(GAUSSIAN_QUADRATURE, TEST_FOURTH_ORDER) {
    constexpr auto lambda = [&](const scalar x, const scalar y) { return x*x * y*y*y*y; };

    const auto result = DefiniteIntegrals::integrate<DefiniteIntegrals::Quadrature<3, 3>>(lambda, {0, 0}, {2, 2});
    ASSERT_EQ(2 * 2 * 2 * 2  * 2 * 2 * 2 * 2. / 15, result);
}

TEST_F(GAUSSIAN_QUADRATURE, TEST_FIFTH_ORDER) {
    constexpr auto lambda = [](const scalar x, const scalar y) { return x*x * y*y*y*y*y; };

    const auto result = DefiniteIntegrals::integrate<DefiniteIntegrals::Quadrature<3, 3>>(lambda, {0, 0}, {2, 2});
    ASSERT_NEAR(2 * 2 * 2 * 2  * 2 * 2 * 2 * 2 * 2. / 18 - result, 0, 4e-15);
}
#endif