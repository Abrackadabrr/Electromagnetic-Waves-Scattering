  ...
  public:
    void SetUp() override {
        const std::string cellsFile =
            "/home/evgen/Education/MasterDegree/thesis/Electromagnetic-Waves-Scattering/tests/meshes_for_tests/lattice_redused/80_cells.csv";
        const std::string nodesFile =
            "/home/evgen/Education/MasterDegree/thesis/Electromagnetic-Waves-Scattering/tests/meshes_for_tests/lattice_redused/320_nodes.csv";
...
TEST_F(DOUBLE_TOEPLITZ_OVER_HOMOGENOUS_STRUCTURE, GMRES_TESTING) {
    ...
    const auto res_toeplitz = solve(Wrappers::MatrixReplacement{test_matrix}, vec, tolerance);
    // Сравниваем решения между собой
    ASSERT_NEAR((res_full - res_toeplitz).norm() / res_full.norm(), 0, 1e-13);
}


#include "geometry/field_over/FieldOverGeometry.hpp"